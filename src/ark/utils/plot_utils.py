...
class MetaclusterColormap:
    ...
    def _metacluster_cmap_generator(self) -> None:
        ...
        # NOTE: check for 'cluster' ensures this file was generated by the interactive
        # visualization
        ...
        metacluster_id_to_name = metacluster_id_to_name.sort_values(
            by=f'{self.cluster_type}_meta_cluster')
        ...
        # need to add black [0,0,0,1] to denote a pixel with no clusters
        # Convert the list of tuples to a numpy array, each index is a color
        mc_colors: np.ndarray = np.concatenate(
            (np.array([[0, 0, 0, 1]]), np.array(list(metacluster_id_to_name['color']))))
        ...
        for index, row in metacluster_id_to_name.reset_index(drop=True).iterrows():
            metacluster_to_index[row[f'{self.cluster_type}_meta_cluster']] = index + 1
    ...
    def assign_metacluster_cmap(self, fov_img: np.ndarray) -> np.ndarray:
        ...
        """
        acceptable_cluster_ids = [
            0] + list(self.metacluster_id_to_name[f'{self.cluster_type}_meta_cluster'])
