...
def create_object_masks(input_image: np.ndarray,
                        ...
                        max_object_area: int = 100000,) -> np.ndarray:
    """
    ...
    # Input validation
    misc_utils.verify_in_list(object_shape=[object_shape_type],
                              object_shape_options=["blob", "projection"])
    ...
    if hole_size is not None:
        ...
        small_holes_block_size: int = get_block_size(block_type="small_holes",
                                                     fov_dim=fov_dim,
                                                     img_shape=img2mask_shape[0])
        img2mask_rm_holes: np.ndarray = morphology.remove_small_holes(
    ...
    region_properties: Dict = measure.regionprops_table(
    ...
    object_masks_df = pd.DataFrame(region_properties)

    # Filter sizes: min_object_area < object_area < max_object_area
    filtered_objects = object_masks_df[object_masks_df["area"].between(
...
def get_block_size(block_type: str, fov_dim: int, img_shape: int) -> int:
    ...
    # Input validation
    misc_utils.verify_in_list(block_type=[block_type],
                              block_types=["small_holes", "local_thresh"])
