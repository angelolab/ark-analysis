...
from segmentation.utils import cell_label_spatial_analysis
import importlib
importlib.reload(cell_label_spatial_analysis)
importlib.reload(spatial_analysis_test)


def make_pheno_codes():
    # Create phenotype list with IDs
    pheno = pd.DataFrame(np.zeros((2, 2)))
    # Assigns the two cel phenotypes to ID 1 and 2
...
def test_visualize_z_scores():
    ...
    plot = cell_label_spatial_analysis.visualize_z_scores(z, pheno_titles)
...
def test_calculate_channel_spatial_enrichment():
    ...
    values, stats = \
        cell_label_spatial_analysis.cell_label_spatial_enrichment(
