...
def test_spatial_analysis():
    ...
    test_closeNum = spatialorgmarkerexpression_utils.spatial_analysis(test_distmat, test_thresholds, test_cellarray)
    ...
    closeNum, closeNumRand, z, muhat, sigmahat, p, h, adj_p, markerTitles = spatialorgmarkerexpression_utils.spatial_analysis(distMatP, marker_thresholds, cellArrayP)
    assert p[0, 1, 0] < .05
    assert z[0, 1] > 0
    #negative enrichment
    ...
    closeNum, closeNumRand, z, muhat, sigmahat, p, h, adj_p, markerTitles = spatialorgmarkerexpression_utils.spatial_analysis(distMatN, marker_thresholds, cellArrayN)
    assert p[0, 1, 1] < .05
    assert z[0, 1] < 0
    #no enrichment
    cellArray = spatialanalysis_utils.make_expression_matrix("none")
    distMat = spatialanalysis_utils.make_distance_matrix("none")
    closeNum, closeNumRand, z, muhat, sigmahat, p, h, adj_p, markerTitles = spatialorgmarkerexpression_utils.spatial_analysis(distMat, marker_thresholds, cellArray)
    assert p[0, 1, 0] > .05
    assert abs(z[0, 1]) < 1
