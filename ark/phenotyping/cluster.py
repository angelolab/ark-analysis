...
def batch_winner(samples, weights):
    ...
    # subtract the weights from each sample individually
    # shape will be (m x n) x r x c
    # (m x n): number of m and n SOM nodes
    # r: number of rows in the pixel matrix
    # c: number of channels (columns) in the pixel matrix
    samples_subtract = samples.T[np.newaxis, np.newaxis, :, :] - weights[:, :, :, np.newaxis]

    # compute the activations, the same as np.linalg.norm without the square roots
    # since we only care about relative magnitude
    activation_map = np.add.reduce(samples_subtract * samples_subtract, axis=2)

    # reshape from (m x n) x r to r x (m x n)
    activation_map = np.swapaxes(activation_map, 0, 2)
