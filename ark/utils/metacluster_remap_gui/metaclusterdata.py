...
class MetaClusterData():
    def __init__(self, path, cluster_filename, pixelcount_filename, output_mapping_filename):
        ...
        self.cluster_pixelcounts = pd.read_csv(self.path / pixelcount_filename).sort_values('cluster').set_index('cluster')
    ...
    def metaclusters(self):
        if not self._cached_metaclusters is None:
            return self._cached_metaclusters
        weighted_clusters = self.clusters.multiply(self.cluster_pixelcounts['count'], axis=0)
        metacluster_pixelcounts = self.cluster_pixelcounts.join(self.mapping).groupby('hCluster_cap').aggregate('sum')
        weighted_metaclusters = weighted_clusters.join(self.mapping).groupby('hCluster_cap').aggregate('sum').divide(metacluster_pixelcounts['count'], axis=0)
