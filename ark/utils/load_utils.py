...
def load_imgs_from_dir(data_dir, files=None, match_suffix=None, trim_suffix=None,
    ...
    if files is None:
        ...
        if match_suffix is not None:
            ...
            imgs = [imgs[i] for i, name in enumerate(filenames) if name.endswith(match_suffix)]
        imgs.sort()
    else:
        imgs = files
        for img in imgs:
